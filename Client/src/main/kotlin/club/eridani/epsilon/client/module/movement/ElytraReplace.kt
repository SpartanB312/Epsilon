...
internal object ElytraReplace : Module(
    ...
    private fun sendEquipNotification() {
        ...
        if (logToChat.value && elytraCount == 1) {
            ChatUtil.sendNoSpamMessage("[ElytraReplace] You equipped your last elytra.")
        } else if (logToChat.value && elytraCount <= logThreshold.value) {
            ChatUtil.sendNoSpamMessage("[ElytraReplace] You have $elytraCount elytra(s) left.")
        }
    }
    ...
    private fun sendFinalElytraWarning() {
        ...
        if (logToChat.value) {
            ChatUtil.sendNoSpamMessage("[ElytraReplace] Your last elytra has reached your durability threshold.")
        }
        if (playSound.value) {
            sendBadAlert()
        }
