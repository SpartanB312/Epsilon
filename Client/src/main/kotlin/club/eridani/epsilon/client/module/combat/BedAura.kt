...
import club.eridani.epsilon.client.util.graphics.ESPRenderer
...
import club.eridani.epsilon.client.util.inventory.inventoryTask
import club.eridani.epsilon.client.util.inventory.operation.swapWith
import club.eridani.epsilon.client.util.items.blockBlacklist
import club.eridani.epsilon.client.util.math.RotationUtils.getRotationTo
import club.eridani.epsilon.client.util.math.RotationUtils.yaw
...
import club.eridani.epsilon.client.util.pause.OffhandPause
import club.eridani.epsilon.client.util.pause.withPause
import club.eridani.epsilon.client.util.spoofUnSneak
...
internal object BedAura : Module(
    ...
    private val damageStep by setting("Damage Step", 2.0f, 0.0f..5.0f, 0.1f, page.atValue(Page.GENERAL) and BedAura::smartDamage)
    ...
    private val slowModeDamage by setting("Slow Mode Damage", 4.0f, 0.0f..10.0f, 0.25f, page.atValue(Page.TIMING) and BedAura::slowMode)
    private val slowDelay by setting("Slow Delay", 250, 0..1000, 5, page.atValue(Page.TIMING) and BedAura::slowMode and { timingMode != TimingMode.SWITCH })
    private val slowPlaceDelay by setting("Slow Place Delay", 250, 0..1000, 5, page.atValue(Page.TIMING) and BedAura::slowMode and { timingMode == TimingMode.SWITCH })
    private val slowBreakDelay by setting("Slow Break Delay", 50, 0..1000, 5, page.atValue(Page.TIMING) and BedAura::slowMode and { smartDamage || timingMode == TimingMode.SWITCH })

    private val forcePlaceBind by setting("Force Place Bind", KeyBind(Keyboard.KEY_NONE){
        if (isEnabled) {
            toggleForcePlace = !toggleForcePlace
           ChatUtil.sendNoSpamErrorMessage("[BedAura] Force placing" + if (toggleForcePlace) " §aenabled" else " §cdisabled")
        }
    ...
    private val forcePlaceDamageBalance by setting("Force Place Damage Balance", 0.0f, -10.0f..10.0f, 0.25f, page.atValue(
        Page.FORCE_PLACE
    ))
    ...
    private val footColor by setting("Foot Color", ColorRGB(255, 160, 255), page.atValue(Page.RENDER) and BedAura::renderFoot)
    private val headColor by setting("Head Color", ColorRGB(255, 32, 64), page.atValue(Page.RENDER) and BedAura::renderHead)
    private val baseColor by setting("Base Color", ColorRGB(32, 255, 32), page.atValue(Page.RENDER) and BedAura::renderBase)
...
//            renderer.render(false)
        ...
        listener<Render2DEvent> {
            ...
            MainFontRenderer.drawString(placeInfo.string, x, y, scale = 2.0f,color = GUIManager.white)
        }
        ...
        BackgroundScope.launchLooping("BedAura", 5L) {
            if (isEnabled) {
                runSafe {
                    runLoop()
                }
    ...
    private inline fun SafeClientEvent.breakBed(placeInfo: PlaceInfo) {
        ...
        player.spoofUnSneak {
            connection.sendPacket(CPacketPlayerTryUseItemOnBlock(placeInfo.bedPosFoot, side, handMode, hitVecOffset.x, hitVecOffset.y, hitVecOffset.z))
        }
        ...
        if (player.dimension == 0 || !player.allSlots.hasItem(Items.BED)) {
            ...
            club.eridani.epsilon.client.util.threads.defaultScope.launch {
                ...
                if (info != null) {
                    ...
                    if (renderFoot) list.add(ESPRenderer.Info(info.boxFoot, footColor, club.eridani.epsilon.client.util.graphics.mask.EnumFacingMask.ALL xor club.eridani.epsilon.client.util.graphics.mask.EnumFacingMask.getMaskForSide(info.side)))
                    if (renderHead) list.add(ESPRenderer.Info(info.boxHead, headColor, club.eridani.epsilon.client.util.graphics.mask.EnumFacingMask.ALL xor club.eridani.epsilon.client.util.graphics.mask.EnumFacingMask.getMaskForSide(info.side.opposite)))
                }
