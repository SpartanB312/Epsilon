...
import club.eridani.epsilon.client.util.inventory.slot.chestSlot
...
import club.eridani.epsilon.client.module.player.WTap.max
import club.eridani.epsilon.client.module.player.WTap.min
...
internal object ElytraFlight : Module(
    ...
    private val threshold by setting("Warning Threshold", 5, 1..50, 1) { page.value == Page.GENERIC_SETTINGS && durabilityWarning0.value }
...
//    private val highPingOptimize0 = setting("High Ping Optimize", false) { page.value == Page.GENERIC_SETTINGS && easyTakeOff0.value }
//    private val highPingOptimize by highPingOptimize0
    private val minTakeoffHeight by setting("Min Takeoff Height", 0.5f, 0.0f..1.5f, 0.1f) { page.value == Page.GENERIC_SETTINGS && false }
    ...
    private enum class ElytraFlightMode(val displayName: CharSequence) {
        BOOST("Boost"), CONTROL("Control"), CREATIVE("Creative"), PACKET("Packet")
    }
        ...
        if (elytraIsEquipped) {
            ...
            if (!player.onGround && oldDurability != elytraDurability) {
                if (durabilityWarning && elytraDurability > 1 && elytraDurability < threshold * armorSlot.maxDamage / 100) {
                    ...
                    ChatUtil.sendNoSpamWarningMessage("Warning: Elytra has " + (elytraDurability - 1) + " durability remaining")
                } else if (elytraDurability <= 1 && !outOfDurability) {
                    outOfDurability = true
                    if (durabilityWarning) {
                        mc.soundHandler.playSound(PositionedSoundRecord.getRecord(SoundEvents.ENTITY_EXPERIENCE_ORB_PICKUP, 1.0f, 1.0f))
                        ChatUtil.sendNoSpamWarningMessage("Elytra is out of durability, holding player in the air")
                    }
                }
            }
    ...
    private fun SafeClientEvent.landing(event: PlayerTravelEvent) {
        when {
            player.onGround -> {
                ChatUtil.sendNoSpamWarningMessage("Landed!")
                autoLanding = false
                return
            }
    ...
    private fun SafeClientEvent.takeoff(event: PlayerTravelEvent) {
        ...
        val timerSpeed = 200.0f
        val height = minTakeoffHeight
...
//            if (LagNotifier.paused && LagNotifier.pauseTakeoff && player.posY - world.getGroundLevel(player) > 4.0f) holdPlayer(event) /* Holds player in the air if server is lagging and the distance is enough for taking fall damage */
//            reset(player.onGround)
//            return
//        }

        if (player.motionY < 0 || player.motionY < -0.02) {
            if (closeToGround) {
    ...
    private fun shouldSwing(): Boolean {
    ...
    private fun SafeClientEvent.spoofRotation() {
        ...
        var rotation = Vec2f(player.rotationYaw, player.rotationPitch)
