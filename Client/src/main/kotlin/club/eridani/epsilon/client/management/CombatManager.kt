...
import club.eridani.epsilon.client.util.combat.CrystalUtils.blockPos
import club.eridani.epsilon.client.util.combat.CrystalUtils.canPlaceCrystal
import club.eridani.epsilon.client.util.combat.DamageReduction
...
object CombatManager {
    ...
    init {
                ...
                when (event.packet) {
                    is SPacketSoundEffect -> {
                        if (event.packet.category == SoundCategory.BLOCKS && event.packet.sound == SoundEvents.ENTITY_GENERIC_EXPLODE) {
                            val list = crystalList.asSequence().map(Pair<EntityEnderCrystal, CrystalDamage>::first).filter { it.getDistanceSq(event.packet.x, event.packet.y, event.packet.z) <= 144.0 }.onEach(Entity::setDead).toList()
                    ...
                    is SPacketEntityMetadata -> {
                        ...
                        val entry = event.packet.dataManagerEntries.find {
                            it.isDirty && it.key == runCatching { AccessorEntityLivingBase.epsilonGetHealthDataKey() }.getOrNull()
                        } ?: return@safeListener
        ...
        safeParallelListener<TickEvent> {
            runCatching {
                val trackerSelf = trackerSelf?.takeIf { it.entity === player } ?: MotionTracker(player)
    ...
    fun getCrystalDamage(crystal: EntityEnderCrystal) = crystalMap0[crystal] ?: getCrystalDamage(crystal.blockPos)

    fun getCrystalDamage(blockPos: BlockPos) = contextSelf?.let { contextSelf ->
    ...
    fun getDamageReduction(entity: EntityLivingBase) = damageReductions[entity]
    ...
    private fun SafeClientEvent.updateCrystalDamage() {
        ...
        if (flag1 || flag2) {
            ...
            if (flag1) {
                placeJob = SpartanCore.launch {
                    updatePlaceMap(contextSelf, contextTarget, remove)
                    updatePlaceList()
                }
            }
            if (flag2) {
                crystalJob = SpartanCore.launch {
    ...
    private fun SafeClientEvent.updatePlaceMap(contextSelf: CalcContext, contextTarget: CalcContext?, remove: Boolean) {
        ...
        placeMap0.values.removeIf { crystalDamage ->
            remove && (crystalDamage.crystalPos.squareDistanceTo(eyePos) > MAX_RANGE_SQ || !canPlaceCrystal(crystalDamage.blockPos, null) || contextTarget != null && (crystalDamage.crystalPos.squareDistanceTo(contextTarget.predictPos) > MAX_RANGE_SQ || !contextTarget.checkColliding(crystalDamage.crystalPos)))
        }
    ...
    private fun SafeClientEvent.updateCrystalMap(contextSelf: CalcContext, contextTarget: CalcContext?, remove: Boolean) {
        val eyePos = player.eyePosition
        val mutableBlockPos = BlockPos.MutableBlockPos()
    ...
    private fun updatePlaceList() {
        placeList = placeMap.values.sortedByDescending { it.targetDamage }
    }

    private fun updateCrystalList() {
        crystalList = crystalMap.entries.map { it.toPair() }.sortedByDescending { it.second.targetDamage }
    }
