...
object ClassUtil {
    ...
    fun findClasses(
        ...
        if (isJar) {
            val path = root.path
            val reformattedPath = path
                .substring(6, path.length - (packagePath.length + 2))
                .replace("%20", " ")
            val file = File(reformattedPath)
            findClassesInJar(classLoader, file, packagePath, predicate, classes)
        } else {
