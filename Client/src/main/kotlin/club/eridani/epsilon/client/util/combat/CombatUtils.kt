...
import club.eridani.epsilon.client.util.inventory.slot.filterByStack
import club.eridani.epsilon.client.util.inventory.slot.hotbarSlots
import club.eridani.epsilon.client.util.items.attackDamage
...
object CombatUtils {
    ...
    fun SafeClientEvent.equipBestWeapon(preferWeapon: PreferWeapon = club.eridani.epsilon.client.util.combat.CombatUtils.PreferWeapon.NONE, allowTool: Boolean = false) {
        player.hotbarSlots.filterByStack {
            ...
            when {
                preferWeapon == club.eridani.epsilon.client.util.combat.CombatUtils.PreferWeapon.SWORD && item is ItemSword -> damage * 10.0f
                preferWeapon == club.eridani.epsilon.client.util.combat.CombatUtils.PreferWeapon.AXE && item is ItemAxe -> damage * 10.0f
                else -> damage
            }
        }?.let {
            swapToSlot(it)
    ...
    init {
        safeParallelListener<TickEvent>(alwaysListening = true) {
            for (entity in world.loadedEntityList) {
                ...
                cachedArmorValues.getOrPut(entity, CombatUtils::ArmorInfo).update(armorValue, toughness)
            }
        }
